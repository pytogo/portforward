...
func PortForward(namespace *C.char, podName *C.char, fromPort, toPort int) {
	...
	if err := forward(dialer, ports, stopChan, readyChan); err != nil {
		panic(err)
	}
}
...
// forward runs the port-forwarding.
func forward(dialer httpstream.Dialer, ports string, stopChan, readyChan chan struct{}) error {
